...
if ($npm == "" || $password == "")
{
   echo"<script>alert('Maaf, npm atau password anda masih kosong',document.location.href='javascript:history.back(0)')</script>";
}
else
{
include("koneksi.php");
$sql="select * from pendaftaran where (status_pendaftaran = 'Y') and (npm = '".$_POST['npm']. "') and (password = '".($_POST['password']). "')";
$query=mysql_query($sql);
$row=mysql_fetch_array($query);
if($row[npm]==$npm and $row[password]==$password)
{
session_start();
...
{
 echo"<script>alert('Maaf, npm dan password anda salah silahkan login kembali dengan benar',document.location.href='?page=16')</script>";
}}
?>